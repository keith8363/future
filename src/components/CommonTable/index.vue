<template>
	<div class='common-table'>
		<el-table ref="Commontable" :class="{'currentRowPointer': table.highlightCurrentRow}" v-loading="table.loading" :row-style="rowStyle" :cell-style="cellStyle" :cell-class-name="handleCellClassName" :header-cell-style="headerStyle" :header-cell-class-name="headerCellClassName" :row-class-name="tableRowClassName" :show-summary="table.hasShowSummary" :summary-method="getSummaries" :span-method="table.getSpanMethod" :show-header="table.showHeader" :data="table.data" :height="table.tableHeight" tooltip-effect="dark myTooltips" :border="table.border" :style="table.style" :stripe="table.stripe" :tree-props="table.treeProps ? table.treeProps : {children: 'children', hasChildren: 'hasChildren'}" :row-key="table.rowKey ? table.rowKey : 'id'" lazy :fit="true" :load="table.load?table.load:null" header-row-class-name="thClassName" @selection-change="handleSelectionChange" @select="select" @cell-mouse-enter="MouseEnter" @cell-mouse-leave="MouseOut" @sort-change="sortChange" @row-click="rowClick" @select-all="selectAll" @header-click="handleHeaderClick" :default-sort="table.defaultSort || {}" :highlight-current-row="table.highlightCurrentRow">
			<el-table-column v-if="table.hasSelect" type="selection"  :align="'center'"  width="55" :reserve-selection="true" :selectable="selectable"></el-table-column>
			...
			<template v-for="item in table.tr">
				<el-table-column v-if="item.show !== false && item.show === 'template'" :key="item.id+item.label" :show-overflow-tooltip="item.hiddenToolTip ? !item.hiddenToolTip : true" :label="item.label" :prop="item.prop" :align="item.align" :sortable="item.sortable" :class-name="item.className ? item.className : ''" :width="item.width ? item.width : ''" :min-width="item.minWidth ? item.minWidth : ''" :fixed="item.fixed || false ">
					<template #default="scope">
						<slot :name="item.prop" :value="scope.row[item.prop]" :row="scope.row" :rowIndex="scope.$index" ></slot>
					</template>
				</el-table-column>
				...
				<el-table-column v-else-if="item.show !== false && item.show === 'isMultipleHeader'" :key="item.id + item.prop" :show-overflow-tooltip="item.showToolTip ? item.showToolTip : true" :label="item.label" :prop="item.prop" :sortable="item.sortable" :align="item.align ? item.align : 'center'" :class-name="item.className ? item.className : ''" :width="item.width ? item.width : ''" :min-width="item.minWidth ? item.minWidth : ''" :fixed="item.fixed || false ">
					<template v-for="(subItem) in item.child">
						...
						<el-table-column v-else :key="subItem.id + subItem.prop" :show-overflow-tooltip="subItem.showToolTip ? subItem.showToolTip : true" :label="subItem.label" :prop="subItem.prop" :sortable="subItem.sortable" :align="subItem.align ? subItem.align : 'center'" height="80" :class-name="subItem.className ? subItem.className : ''" :width="subItem.width ? subItem.width : ''" :min-width="subItem.minWidth ? subItem.minWidth :  ''" :fixed="subItem.fixed || false ">
							<template #default="scope">
								<slot :name="subItem.prop" :value="scope.row[subItem.prop]" :row="scope.row"></slot>
							</template>
						</el-table-column>
					</template>
				</el-table-column>
			...
			<template v-slot:empty>
				...
				<div class="noData" v-if="!table.loading"><img src="@/assets/header/noData.png"><span class="noDataText">{{table.emptyText || '暂无数据'}}</span></div>
				</p>
			</template>
		...
		<div v-if="table.hasPagination" class="pagination_row">
			<!-- 分页组件 -->
			<el-pagination class="el-pagination-style mt-4" background  :total="table.totalCount" :page-sizes="table.pageSizes" :page-size="table.pageSize" :layout="table.layout ? table.layout : 'total, prev, pager, next, sizes, jumper'" :current-page="table.page" @size-change="handleSizeChange" @current-change="handleCurrentChange"></el-pagination>
		</div>
	</div>
</template>
...
export default defineComponent({
	...
	setup(props, { emit }) {
		...
		const handleCellClassName = ({ columnIndex }) => {
			if (
				!props.table.highlightCurrentRow &&
				columnIndex == props.table.heightLight
			) {
				return 'height_light'
			}
		}
		...
		const rowClick = (Row, Event, Column) => {
			...
			Commontable.value?.setCurrentRow(Row)

			emit('onRowClick', data)
		}

		const btnClick = (scope: any) => {
			if (scope.row.link && props.table.openDetailName) {
				window.open(scope.row.link, scope.row[props.table.openDetailName])
			}
		}
		...
		const tableRowClassName = ({
			...
			if (!props.heightlight && props.heightlight !== 0) {
				if (rowIndex % 2 !== 0) {
					return 'warning-row-odd-row' // 奇数行的类名
				}
				return '' // 偶数行的类名为空
			} else {
				if (rowIndex === props.heightlight) {
					return 'success-row'
				} else {
					return ''
				}
		...
		//  取消表格当前选中行效果
		const cancelSetCurrentRow = () => {
			Commontable.value?.setCurrentRow()
		}
		...
		onMounted(() => {
			watch(
				() => props.table.data,
				() => {
					if (props.table.heightLight) {
						setTimeout(() => {
							Commontable.value?.setCurrentRow(
								props.table.data[props.table.heightLight]
							)
						}, 500)
					}
				},
			...
			watch(
				() => props.table.heightLight,
				(val) => {
					if (!val && val !== 0) return
					setTimeout(() => {
						Commontable.value?.setCurrentRow(props.table.data[val])
					}, 500)
				},
				{
					deep: true,
			...
			watch(
				...
				(val) => {
					if (
						...
						props.table.highlightCurrentRow &&
						!props.table.heightLight &&
						!props.table.notOneSelect
					) {
						setTimeout(() => {
							Commontable.value?.setCurrentRow(props.table.data[0])
						}, 500)
					}
				},
				{
...
:deep(.currentRowPointer .el-table__body .el-table__row:hover) {
	cursor: pointer;
	background: #dce7ff; /* 设置你想要的高亮颜色 */
}
...
/* 如果你还想覆盖鼠标悬停时的样式，可以添加以下样式 */
:deep(.el-table .el-table__row--hover) {
	background: #dce7ff; /* 设置你想要的悬停颜色 */
}
