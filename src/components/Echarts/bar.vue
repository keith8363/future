...
import { strlen, throttle } from '@/utils/debounce'
...
const initChat12 = () => {
	const options = {
		tooltip: {
			...
			formatter: (data: any) => {
				let str = `${data[0].name}<br>`
				let toolTipData = opts.toolTipData
				...
				let value = ''
				if (newData.length) {
					let name = newData[0].seriesName
					if (name === '增加') {
						let auxdataUp = data.find((item) => item.seriesName === '增加辅助')
						value = toolTipData[auxdataUp.dataIndex]
					} else if (name === '减少') {
						let auxdataDown = data.find(
							(item) => item.seriesName === '减少辅助'
						)
						value = toolTipData[auxdataDown.dataIndex]
					} else {
				...
				newData.map((a: any) => {
					str += `${a.marker}<span style="color:#fff"><span style="width:5em;display:inline-block;">${a.seriesName}</span>`
					str += `<span style="width:10em;padding-right:10px;text-align:right;display:inline-block;font-weight:600">${value}${
		...
		series: opts.data.map((a: any, index) => {
			return {
				...
				label: {
					normal: {
						show: a.label.normal.show,
						position: a.label.normal.position,
						formatter: function (params) {
							return opts.toolTipData[params.dataIndex]
							// if (params.seriesName === '增加') {
							// 	let auxdataUp = opts.data.find(
							// 		(item) => item.name === '增加辅助'
							// 	)
							// 	return addDecimalNumbers(
							// 		params.value,
							// 		auxdataUp.data[params.dataIndex],
							// 		2
							// 	)
							// } else if (params.seriesName === '减少') {
							// 	let auxdataDown = opts.data.find(
							// 		(item) => item.name === '减少辅助'
							// 	)
							// 	return subtractDecimalNumbers(
							// 		params.value,
							// 		auxdataDown.data[params.dataIndex],
							// 		2
							// 	)
							// }
						},
	...
	const options = {
    	...
    	formatter: (params) => {
				return `<div style="display:flex;align-item:center;justify-content:space-between;width:${fontSizeWidth(100)};height:${fontSizeWidth(32)};">
					...
					<span>${params.value}${opts.unit}</span>
					</div>`
    	}
  	},
		xAxis:{
			type:'category',
			data:opts.data.map(i=>i.label),
			axisTick:{
				show:false
			},
			axisLabel:{
		...
		series:[
			{
				data:opts.data.map(i=>i.value),
				type:'pictorialBar',
				symbol: 'path://M0,10 L10,10 C5.5,10 5.5,5 5,0 C4.5,5 4.5,10 0,10 z',
				legendHoverLink:true, //图例hover高亮
				showBackground:true,
				showBackgroundStyle:{
					color: '#c0daff17'
				},
				barWidth:fontSizeWidth(68),
				largeThreshold:400,
				itemStyle:{
					color:{
					  type: 'linear',
					  ...
					  y2: 1,
					  colorStops: [{
					      offset: 0, color: '#2169d145' // 0% 处的颜色
					  }, {
					      offset: 1, color: '#21A5D1' // 100% 处的颜色
					  }],
					  global: false // 缺省为 false
					},
				},
