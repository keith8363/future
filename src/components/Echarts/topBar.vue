...
const transOptions = (result) => {
	return {
		...
		dataZoom: [
			{
				show: result.length > (props.type === 1?4:6),
				startValue: 0, // 数据窗口范围的起始数值
				endValue: (props.type === 1?4:6), // 数据窗口范围的结束数值（一页显示多少条数据）
				type: 'slider',
				maxValueSpan: (props.type === 1?4:6), // 显示数据的条数(默认显示10个)
				handleSize: 0, // 滑动条的 左右2个滑动条的大小
				height: props.showZoomBar?'100%':'0', // 组件高度
				width: props.showZoomBar?'10':'0',
				left: props.showZoomBar?'98%':'96%', // 左边的距离
				right: 4, // 右边的距离
				top:5,
				bottom: 10, // 上边边的距离
				borderColor: 'transparent',
				fillerColor: '#dedfe0', // 滑动块的颜色
