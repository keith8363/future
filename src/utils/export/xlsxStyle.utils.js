...
export default {
	...
	initAllCell: function(workBook, sheetName) {
		...
		var sc = utils.decode_cell(startCell).c;
		var sr = utils.decode_cell(startCell).r;
		var ec = utils.decode_cell(endCell).c;
		var er = utils.decode_cell(endCell).r;
		var isExist;

		for (let c = sc; c <= ec; c++) { //初始化所有单元格
			for (let r = sr; r <= er; r++) {
				var temp = utils.encode_cell({
					c: c,
					r: r
				});
				isExist = false;
				...
				if (!isExist) { //单元格不存在则新建单元格
					utils.sheet_add_aoa(workBook.Sheets[sheetName], [
						['']
					], {
						origin: temp
					});
	...
	*/
	...
	setTitleStylesDefault: function(workBook, sheetName) {
		for (let cell in workBook.Sheets[sheetName]) {
			let row = cell.substr(1);
			if (row == '1') {
				this.setFillFgColorRGB(workBook, sheetName, cell, '79bbff');
	...
	setEvenRowColorGrey: function(workBook, sheetName) {
		for (let cell in workBook.Sheets[sheetName]) {
			if (cell != '!cols' && cell != '!merges' && cell != '!ref') {
				...
				if (row % 2 == 1 && row!==1) {
					this.setFillFgColorRGB(workBook, sheetName, cell, 'e6e5f8');
				}
