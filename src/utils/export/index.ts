...
import XSU from './xlsxStyle.utils.js';


const setTableStyle = (wb:any) => {
  ...
  // ----------------- 字体样式 ----------------
  XSU.setFontTypeAll(wb,sheetName,'DIN-Bold')

  // ----------------- 配置表格列宽 --------------
  // 获取表格对象
  const obj = wb['Sheets']['Data']
  const attrArr = Object.keys(obj).sort((a,b)=>{
    return   getLetterIndex(a.slice(0,1)) - getLetterIndex(b.slice(0,1))
  })
  // 获取表格对象最后一个属性名称首字母
  const name = attrArr.slice(-1).pop()?.slice(0,1) || 'a'
  // 判断该首字母为英文字母表的第几个
  const index = getLetterIndex(name)
  const arr = new Array()
  let i = 0
  while(i<index){
    const item = i === 0 ? 200 : 150
    arr.push({wpx:item})
    i++
  }
  XSU.setColWidth(wb,sheetName,arr)
}

const getLetterIndex = (letter:string) => {
  const lowerCaseLetter = letter.toLowerCase(); // 转换为小写，统一处理'A-Z'和'a-z'
  if (/[A-Za-z]/.test(letter)) { // 判断是否为英文字母
    return lowerCaseLetter.charCodeAt(0) - 96; // 英文字母'A'和'a'的ASCII码值分别为65和97，所以减去96得到索引
  }
...
export default function(mergedCells:Array<object>,list:Array<object>,title:string,props2?:Array<string>,decimalPlace?:number) {
  ...
  var wbout = write(wb,{bookType:'xlsx', bookSST:false, type:'binary'});
    
  saveAs(new Blob([XSU.s2ab(wbout)],{type:""}),`${title}.xlsx`)
}